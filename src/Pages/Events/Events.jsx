...
const Events = () => {
  ...
  //           setBackgroundImage(`https://new-admin-backend.vercel.app/static/images/${events[0].background_image || events[0].image}`);
  //         }
  //       }
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        // Reverse order so last comes first
        const reversedEvents = [...events].reverse();
  
        setAnnouncementList(reversedEvents);
        setAllEvents(reversedEvents);
  
        // Set the first event as featured if events exist
        if (reversedEvents.length > 0) {
          setEvent(reversedEvents[0]);
  
          // Set background image from first event
          if (reversedEvents[0].background_image || reversedEvents[0].image) {
            setBackgroundImage(
              `https://new-admin-backend.vercel.app/static/images/${reversedEvents[0].background_image || reversedEvents[0].image}`
            );
          }
        }
  
        console.log('Fetched events:', reversedEvents);
      } catch (error) {
        console.error('Failed to fetch events:', error);
        setAnnouncementList([]);
        setAllEvents([]);
    ...
    const interval = setInterval(() => {
      setCurrentEventIndex((prevIndex) => {
        ...
        if (nextEvent && (nextEvent.background_image || nextEvent.image)) {
          setBackgroundImage(`https://new-admin-backend.vercel.app/static/images/${nextEvent.background_image || nextEvent.image}`);
        }
  ...
  const handleEventClick = (selectedEvent, index) => {
    ...
    if (selectedEvent && (selectedEvent.background_image || selectedEvent.image)) {
      setBackgroundImage(`https://new-admin-backend.vercel.app/static/images/${selectedEvent.background_image || selectedEvent.image}`);
    }
              ...
              <div className="Event_subContainer2_div1_cardContainer">
                {allEvents.map((eventItem, index) => (
                  ...
                  >
                    {eventItem.image ? (
                      <img
                        src={`https://new-admin-backend.vercel.app/static/images/${eventItem.image}`}
                        alt={eventItem.title}
